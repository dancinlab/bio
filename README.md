...
| `quantum` | computation | VERIFIED (H₂ 6-Pauli / LiH path) + pocket-scale (F-Q-6-D) + library-ranking (F-Q-6-F) + **GATE-26-2 v2 ✅ cycle-30+++** | ✅ **~99%** — F-Q-1…5 + F-Q-EXT-1…6+ + **F-Q-6-D PASS** (Mpro pocket VQE sub-µHa, `tests/mpro_pocket_vqe_v7.py`) + **F-Q-6-F PASS** (5-warhead library ranking, `tests/mpro_warhead_library_vqe_v7.py`) + **GATE-26-2 hexa-bio-side ✅ CLOSED 2026-05-12 cycle-30+++**: hexa-meta `formal/lean4/` **4 axes PROVEN against WEAVE-semantics v2** (sorry-count=0, kernel-checked on lean4 v4.30.0-rc2 + Mathlib SHA pinned; `lake build N6` → 1919/1919 jobs PASS; hexa-meta commits `a9b5722` → `350798c` → `79bb661` → `2c68bea`); Axis 2 ℝ-valued Landauer + reversible-merge composition; Axis 3 exponential-in-depth Π^p_2 worst case; Axis 4 Finset-backed disclosure with seal-on-first-disclosure semantics. v2 PASS EXCEEDS v2.0.0 GATE-26-2 cert-strength. State ref v2 + `lean4_proof_witness_emit.py` `--refresh` from hexa-meta main; Π¹₁-CA₀ → `decide`/RCA₀-level re-scope for the finitary slice (per `docs/closure_100_research_2026_05_12.md` §C). Remaining: v3 stretch only (parametrised kT / recursive WellFoundedRelation verifier / polymorphic carrier; cycle-30++++, NOT a v1.x or v2.0.0 blocker) + MechVerif legacy sorries (FROZEN in nexus/canon-infra) | (b) v3 stretch | VQE (H₂ 0.4 µHa, LiH 1.41 mHa) + Mpro pocket VQE + 5-warhead library ranking + 11-drug pocket library + ML pilots |
...
- **(b) v2 formal semantics** — cycle-30++/+++ Lean / Mathlib work; tracked in [`.roadmap.lean4_formal`](.roadmap.lean4_formal). ✅ **v2 4-axis promotion COMPLETE 2026-05-12 cycle-30+++** — all four WEAVE-mechanical axes PROVEN against WEAVE-semantics v2 (`lake build N6` → 1919/1919 jobs PASS); v3 stretch items (parametrised kT, recursive WellFoundedRelation verifier, polymorphic disclosure carrier) deferred to cycle-30++++, **NOT a v1.x or v2.0.0 blocker** (§B). MechVerif legacy + Theorem B residual sorries remain FROZEN in legacy-canon.
