...
| `quantum` | computation | VERIFIED (H₂ 6-Pauli / LiH path) + pocket-scale (F-Q-6-D) + library-ranking (F-Q-6-F) + **GATE-26-2 v3 ✅ cycle-30++++** | ✅ **~99%** — F-Q-1…5 + F-Q-EXT-1…6+ + **F-Q-6-D PASS** (Mpro pocket VQE sub-µHa, `tests/mpro_pocket_vqe_v7.py`) + **F-Q-6-F PASS** (5-warhead library ranking, `tests/mpro_warhead_library_vqe_v7.py`) + **GATE-26-2 hexa-bio-side ✅ CLOSED 2026-05-12 cycle-30++++**: hexa-meta `formal/lean4/` **3 of 4 axes at maximum semantics — Axis 1 REAL + Axes 2/4 v3 + Axis 3 v2** (sorry-count=0, kernel-checked on lean4 v4.30.0-rc2 + Mathlib SHA pinned; `lake build N6` → 1919/1919 jobs PASS; hexa-meta commits `a9b5722` → `350798c` → `79bb661` → `2c68bea` → `9e44e75`); Axis 2 kT opaque positive ℝ via section variable + `[Fact (0 < kT)]` (was hard-coded kT:=1 in v2); Axis 4 polymorphic `ClosureCert (α : Type) [DecidableEq α]` (was Finset Nat in v2); Axis 3 stays at v2 exp-in-depth Π^p_2 worst case (v3 recursive WellFoundedRelation verifier is hardest deferred stretch). v3 PASS for Axes 2+4 EXCEEDS v2.0.0 GATE-26-2 cert-strength. State ref v2 + `lean4_proof_witness_emit.py` `--refresh` from hexa-meta main; Π¹₁-CA₀ → `decide`/RCA₀-level re-scope for the finitary slice (per `docs/closure_100_research_2026_05_12.md` §C). Remaining: Axis 3 v3 + v4 stretches (cycle-30+++++, NOT a v1.x or v2.0.0 blocker) + MechVerif legacy sorries (FROZEN in nexus/canon-infra) | (b) v3/v4 stretch | VQE (H₂ 0.4 µHa, LiH 1.41 mHa) + Mpro pocket VQE + 5-warhead library ranking + 11-drug pocket library + ML pilots |
...
- **(b) v2/v3 formal semantics** — cycle-30++/+++/++++ Lean / Mathlib work; tracked in [`.roadmap.lean4_formal`](.roadmap.lean4_formal). ✅ **3 of 4 axes at maximum semantics 2026-05-12 cycle-30++++** — Axis 1 REAL-SEMANTICS PROVEN, Axes 2 + 4 v3 PROVEN (kT parametric + polymorphic carrier), Axis 3 v2 PROVEN (exp-in-depth Π^p_2); `lake build N6` → 1919/1919 jobs PASS. Remaining: Axis 3 v3 (recursive WellFoundedRelation verifier — hardest stretch) + v4 stretches (energy substrate `[OrderedAddCommGroup E]`, payload `[CommutativeMonoid β]`) deferred to cycle-30+++++, **NOT a v1.x or v2.0.0 blocker** (§B). MechVerif legacy + Theorem B residual sorries remain FROZEN in legacy-canon.
