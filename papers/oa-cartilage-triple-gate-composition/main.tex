...
\caption{The Triple Gate of cartilage regeneration (fal.ai-generated cover). A
single injected depot must pass three sequential gates left to right ---
delivery/penetration ($\delta$) through the charged cartilage matrix,
senescent-cell clearance ($\varphi$), and endogenous regeneration ($A$). Only
when all three close does the ceiling clear the $0.90$ functional-restoration
bar; senescent-cell clearance alone saturates at $0.755$ (closed-negative).}
\label{fig:cover}
\end{figure}
...
\clearpage % flush all trailing appendix figures before references, so the paper ends on the bibliography
