...
- **(b) v2/v3 formal semantics / cycle-30++/+++/++++ stretch** — Lean/Mathlib full-WEAVE-algebra work; tracked in [`.roadmap.lean4_formal`](.roadmap.lean4_formal). ✅ **3 of 4 axes at maximum semantics 2026-05-12 cycle-30++++** — Axis 1 REAL, Axes 2 + 4 v3 (hexa-meta `9e44e75`: kT parametric via section variable + `[Fact (0 < kT)]` + polymorphic `ClosureCert (α) [DecidableEq α]`), Axis 3 v2 (`2c68bea`: exp-in-depth Π^p_2 worst case); `lake build N6` → 1919/1919 jobs PASS. Axis 3 v3 (recursive WellFoundedRelation verifier — hardest stretch) + v4 stretches deferred to cycle-30+++++, **NOT a v1.x or v2.0.0 blocker** ([`CLOSURE_RESIDUAL_BACKLOG.md`](CLOSURE_RESIDUAL_BACKLOG.md) §B).
...
| **quantum** | computation | 🟢 VERIFIED (H₂ 6-Pauli / LiH path) + **pocket-scale ✅** (F-Q-6-D) + **library-ranking ✅** (F-Q-6-F) + **GATE-26-2 v3 ✅ cycle-30++++** | F-Q-1…5 PASS · F-Q-EXT-1…6+ PASS · **F-Q-6-D PASS** (Mpro pocket cluster, sub-µHa) · **F-Q-6-F PASS** (5-warhead VQE library ranking, all 11 fragments VQE=CASCI sub-µHa) · **GATE-26-2 hexa-bio-side CLOSED cycle-30++++** | (n/a) | (n/a) | **✅ ~99% (v1.x closure-grade, in-repo) — F-Q-6 / L3 ✅ + Phase D / F-Q-6-F ✅ CLOSED 2026-05-12; L4 single-residue subsumed; **GATE-26-2 hexa-bio-side ✅ CLOSED 2026-05-12 cycle-30++++**: hexa-meta `formal/lean4/` **3 of 4 axes at maximum semantics — Axis 1 REAL + Axes 2/4 v3 + Axis 3 v2** (sorry-count=0; kernel-checked on lean4 v4.30.0-rc2 + Mathlib SHA pinned; `lake build N6` → 1919/1919 jobs PASS; commits hexa-meta `a9b5722` → `350798c` → `79bb661` → `2c68bea` → `9e44e75`): Axis 2 kT opaque positive ℝ via section variable + `[Fact (0 < kT)]` (was hard-coded kT:=1 in v2), Axis 3 exponential-in-depth Π^p_2 worst case (v3 recursive verifier deferred), Axis 4 polymorphic `ClosureCert (α : Type) [DecidableEq α]` (was Finset Nat in v2); **v3 PASS for Axes 2+4 EXCEEDS v2.0.0 GATE-26-2 cert-strength**. legacy-canon `lean4-n6/N6/` Theorem B (σ·φ=n·τ⟺n=6) ESSENTIALLY FULLY PROVEN (FROZEN at canon retirement 2026-05-11, ~4473 ln, ~2 sorry, ~99.99%); state ref v2 + `lean4_proof_witness_emit.py` `--refresh` from hexa-meta main + `run_all.sh`-wired; Π¹₁-CA₀ → `decide`/RCA₀-level re-scope for the finitary slice. 잔여 = Axis 3 v3 (recursive WellFoundedRelation verifier — 최고 난이도 stretch) + v4 stretches per axis 모두 cycle 30+++++, **NOT a v1.x or v2.0.0 blocker** + MechVerif legacy sorries (FROZEN in nexus/canon-infra) — 둘 다 v1.x · v2.0.0 비차단** |
...
4. ~~**QUANTUM** — long pole~~ — **F-Q-6 / L3 ✅ + Phase D / F-Q-6-F ✅ CLOSED 2026-05-12** (`tests/mpro_pocket_vqe_v7.py` + `tests/mpro_warhead_library_vqe_v7.py`; all VQE=CASCI sub-µHa). L4 single-residue subsumed. **GATE-26-2 hexa-bio-side ✅ CLOSED 2026-05-12 cycle-30++++**: canon RETIRED 2026-05-11 → split into hexa-meta (active) + nexus/canon-infra/legacy-canon (frozen Theorem B / MechVerif). hexa-meta `formal/lean4/` **3 of 4 axes at maximum semantics — Axis 1 REAL + Axes 2/4 v3 + Axis 3 v2** (sorry-count=0, kernel-checked on lean4 v4.30.0-rc2 + Mathlib SHA `f8e537424d154a7eaa025c4abab16c96c626f2e0` pinned via lake-manifest.json; `lake build N6` → 1919/1919 jobs PASS; promotion path hexa-meta `ac97a63` → `a9b5722` v1 baseline → `350798c` Axis 4 v2 → `79bb661` Mathlib SHA pin → `2c68bea` Axes 2+3 v2 → `9e44e75` Axes 2+4 v3): Axis 2 v3 with kT opaque positive ℝ via section variable + `[Fact (0 < kT)]` (was hard-coded `kT := 1`) + landauer_pass_compose/merge/heat_nonneg bonus theorems threading kT through; Axis 3 v2 exponential-in-depth `c.size * 2^q.depth + payload` witness matching Π^p_2 worst case + verifierSteps_mono_depth/two_pow_pos bonuses (v3 recursive verifier deferred — hardest stretch); Axis 4 v3 polymorphic `ClosureCert (α : Type) [DecidableEq α]` (was Finset Nat) with caveat_bag/signer_set/seal-on-first-disclosure snapshots + caveat_bag_invariant/signer_set_invariant/addCaveat_idempotent/signer_set_monotonic bonuses. **v3 PASS for Axes 2+4 EXCEEDS v2.0.0 GATE-26-2 cert-strength** for the WEAVE-mechanical 4-axis consumer contract. legacy-canon `lean4-n6/N6/` Theorem B (σ·φ=n·τ⟺n=6) ESSENTIALLY FULLY PROVEN (FROZEN, ~4473 ln, ~2 sorry, ~99.99%). hexa-bio side: state ref v2 `weave/spec/canon_lean4_state_ref.json` (source = hexa-meta main, axis-record proof_summary curator fields) + `_python_bridge/module/lean4_proof_witness_emit.py` (re-impl, `--refresh` merge-not-replace, emits 4 raw_77 rows, sentinel `__LEAN4_PROOF_WITNESS__ PASS`, run_all.sh-wired). Π¹₁-CA₀ → `decide`/RCA₀-level re-scope for the finitary axis-claims (|S₄|=24, σ(6)=12, master identity; mathlib `Fintype.card_perm` etc. ready). 잔여 = Axis 3 v3 (recursive WellFoundedRelation verifier — 가장 어려운 stretch) + v4 stretches (energy substrate `[OrderedAddCommGroup E]` for Axis 2, payload `[CommutativeMonoid β]` for Axis 4) — 모두 cycle 30+++++, **category (b)** stretch, **NOT a v1.x or v2.0.0 blocker** + MechVerif legacy sorries (FROZEN in nexus/canon-infra, **(b)**) — 둘 다 v1.x·v2.0.0 비차단. (F-Q-6-D *E_int* line — `.roadmap.quantum` rows 96-103 — still software-blocked on D3/basis; not a closure deliverable.)
